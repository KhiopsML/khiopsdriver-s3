...
struct MultiPartFile {
  ...
  explicit MultiPartFile(Aws::String bucket, Aws::String filename,
                         tOffset offset, tOffset common_header_length,
                         Aws::Vector<Aws::String> filenames,
                         Aws::Vector<tOffset> cumulative_sizes,
                         Aws::Vector<Aws::String> etags)
      : bucketname_{std::move(bucket)}, filename_{std::move(filename)},
        offset_{offset}, common_header_length_{common_header_length},
        filenames_{std::move(filenames)},
        cumulative_sizes_{std::move(cumulative_sizes)},
        etags_{std::move(etags)}, total_size_{cumulative_sizes_.back()} {}
};
...
struct WriteFile {
  ...
  WriteFile(Aws::S3::Model::CreateMultipartUploadResult &&create_upload_result)
      : writer_{std::move(create_upload_result)},
        bucketname_{writer_.GetBucket()}, filename_{writer_.GetKey()} {
