...
bool DownloadFileRangeToBuffer(const std::string& bucket_name,
    ...
    if (!reader) {
        error("Error reading object: {}", reader.status().message());
        return false;
    }
    ...
    if (reader.bad()/* || reader.fail()*/) {
        error("Error during read: {} {}", (int)(reader.status().code()), reader.status().message());
        return false;
    }
...
bool UploadBufferToGcs(const std::string& bucket_name,
    ...
    if (!status) {
        error("Error during upload: {} {}", (int)(status.status().code()), status.status().message());
        return false;
    }
...
void FallbackToDefaultBucket(std::string& bucket_name) {
    ...
    critical("No bucket specified, and GCS_BUCKET_NAME is not set!");
}
...
int driver_connect()
{
    ...
    if (!client_response) {
        std::cout << "Failed to create Storage Client: " << (int)(client_response.status().code()) << client_response.status().message() << "\n";
        return false;
    }
...
int driver_fileExists(const char *sFilePathName)
{
    ...
    if (object_metadata) {
        debug("file {} exists!", sFilePathName);
        ...
        error("Error checking object: {}", object_metadata.status().message());
...
{
    debug("fseek {} {} {}", stream, offset, whence);
    ...
    switch (whence) {
        ...
        case std::ios::cur:
        {
            ...
            if (computedOffset < 0) {
                critical("Invalid seek offset {}", computedOffset);
                return -1;
            }
        ...
        default:
            critical("Invalid seek mode {}", whence);
            return -1;
...
{
    debug("fread {} {} {} {}", ptr, size, count, stream);
    ...
    debug("offset = {} toRead = {}", h->offset, toRead);
...
{
    debug("fwrite {} {} {} {}", ptr, size, count, stream);
...
int driver_remove(const char *filename)
{
    ...
    if (!status.ok()) {
        error("Error deleting object: {} {}", (int)(status.code()), status.message());
        return 0;
    }
...
int driver_rmdir(const char *filename)
{
	...
	assert(driver_isConnected());
    debug("Remove dir (does nothing...)");
    return 1;
}

int driver_mkdir(const char *filename)
{
    debug("mkdir {}", filename);
...
{
    debug("copyToLocal {} {}", sSourceFilePathName, sDestFilePathName);
    ...
    if (!reader) {
        error("Error initializing download stream: {} {}", (int)(reader.status().code()), reader.status().message());
        return false;
    }
    ...
    if (!file_stream.is_open()) {
        error("Failed to open local file for writing: {}", sDestFilePathName);
        return false;
    }

    // Read from the GCS object and write to the local file
    std::string buffer(1024, '\0');
    debug("Start reading {}", buffer);
...
{
    debug("copyFromLocal {} {}", sSourceFilePathName, sDestFilePathName);
    ...
    if (!file_stream.is_open()) {
        error("Failed to open local file: {}", sSourceFilePathName);
        return false;
    }
    ...
    if (!writer) {
        error("Error initializing upload stream: {} {}", (int)(writer.metadata().status().code()), writer.metadata().status().message());
        return false;
    }
    ...
    auto status = writer.metadata();
    if (!status) {
        error("Error during file upload: {} {}", (int)(status.status().code()), status.status().message());
